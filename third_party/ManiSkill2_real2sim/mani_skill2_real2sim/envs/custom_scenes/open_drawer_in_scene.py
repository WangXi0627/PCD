...
class OpenDrawerInSceneEnv(CustomSceneEnv):
    ...
    def evaluate(self, **kwargs):
        tcp_to_drawer_dist = np.linalg.norm(self.tcp.pose.p - self.drawer_obj.pose.p)
        near_drawer = (tcp_to_drawer_dist < 0.45)
        qpos = self.art_obj.get_qpos()[self.joint_idx]
        self.episode_stats["qpos"] = "{:.3f}".format(qpos)
        return dict(near_drawer=near_drawer, success=qpos >= 0.15, qpos=qpos, episode_stats=self.episode_stats)
