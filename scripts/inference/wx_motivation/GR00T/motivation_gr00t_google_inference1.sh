...
# ==============================
# GR00T-N1.6 random feature mask Google runner
...
# 让 PCD runner 和本地 GR00T 源码可 import。
...
# random feature mask 不加载 grounded_sam_tracking，可以用 5 并行。
# 如果先小测，可以改成 1。
N_ENVS=5
N_ACTION_STEPS=1
SAVE_VIDEO=False
...
# 更接近 PiZero multi_modal_projector / vlm_layer / action_layer 的 GR00T target。
# 第一轮建议先保留 1-2 个，确认趋势后再扩大。
FEATURE_MASK_TARGETS=(
  # "backbone.model.mlp1"
  "backbone.model.vision_model.vision_model.post_layernorm"
)
...
for TARGET in "${FEATURE_MASK_TARGETS[@]}"; do
  ...
  for KEEP_RATIO in "${FEATURE_MASK_KEEP_RATIOS[@]}"; do
    ...
    for SEED in "${FEATURE_MASK_SEEDS[@]}"; do
      RUN_NAME="groot_random_feature_mask/google_ep${N_EPISODES}_env${N_ENVS}_act${N_ACTION_STEPS}/target=${TARGET_TAG}/keep=${KEEP_TAG}/seed=${SEED}_rescale=${FEATURE_MASK_RESCALE}_video=${SAVE_VIDEO}"
...
echo
echo "========================================"
echo "[All Done] GR00T random feature mask experiments finished."
echo "========================================"